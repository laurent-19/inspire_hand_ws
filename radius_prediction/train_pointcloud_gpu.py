#!/usr/bin/env python3
"""Training script for PointNet with GPU-accelerated data loading."""
...
from radius_prediction.data.pointcloud_dataset_gpu import TactilePointCloudDatasetGPU
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train PointNet radius model (GPU-accelerated)")
    parser.add_argument(
...
def collate_fn_gpu(batch):
    """Custom collate that stacks tensors."""
...
def main():
    ...
    train_dataset = TactilePointCloudDatasetGPU(
        ...
        augment=True,
        device="cpu",  # Load on CPU first to avoid multiprocessing issues
    )
    val_dataset = TactilePointCloudDatasetGPU(
        root_dir=NON_DEFORMABLE_DIR,
        val_object=args.val_object,
        split="val",
        num_points=num_points,
    ...
    # Use more workers and pin memory for fast CPU->GPU transfer
    ...
    print(f"\nStarting training (PointNet GPU-accelerated)...")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_rmse < best_rmse:
            ...
            checkpoint_path = checkpoint_dir / f"best_pointnet_gpu_{args.val_object}.pt"
            torch.save({
                "epoch": epoch,
